...
class CantonsModel extends AppModel {

  function __construct()
  {
    parent::__construct();
    $this->table = "cemaco_clients.geo_canton";    
  }
  ...
  public function ListItems()
  {
    ...
    $sql = "SELECT t.id_canton as id, t.*    
  ...
  private function ListWhere($filter = false)
  {
    ...
    if($text)
      $sql .= " AND ( t.canton like '%{$text}%'  OR t.id_canton = '{$text}') ";   
    if($this->input->post('filter-id'))
      $sql .= " AND t.id_canton = '". $this->input->post('filter-id') ."'";  
    return $sql;
  }  
  ...
  {    
    $sql = "select * from {$this->table} where id_canton = '{$id}'";
  ...
  public function SavePost()
  {
    ...
    if( $this->id )
      $sql = $this->db->update_string($this->table, $data, "id_canton = '{$this->id}'" );
    else
  ...
  public function DataElement( $id = 0, $null = false)
  {
    ...
    if($id)
    {
      $sql = "SELECT t.id_canton as id, t.*      
      FROM {$this->table} as t      
      WHERE t.id_canton = '{$id}' 
      LIMIT 0, 1";
      $ret = $this->db->query($sql)->row_array();
      if($ret) return $ret;
      if($null) return false;
    }    
    $ret['canton'] = $this->input->post() ? $this->input->post('canton') : '';
    return $ret;
  }

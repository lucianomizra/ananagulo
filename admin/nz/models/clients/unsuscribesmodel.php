...
class UnsuscribesModel extends AppModel {

  function __construct()
  {
    parent::__construct();
    $this->table = "cemaco_clients.unsuscribe";    
  }
  ...
  public function ListItems()
  {
    ...
    $sql = "SELECT t.id_unsuscribe as id, t.*    
  ...
  private function ListWhere($filter = false)
  {
    ...
    if($text)
      $sql .= " AND ( t.mail like '%{$text}%'  OR t.id_unsuscribe = '{$text}') ";   
    if($this->input->post('filter-id'))
      $sql .= " AND t.id_unsuscribe = '". $this->input->post('filter-id') ."'";  
    return $sql;
  }  
  ...
  {    
    $sql = "select * from {$this->table} where id_unsuscribe = '{$id}'";
    $row = $this->db->query($sql)->row_array();  
    if(!$row) return false;
    unset($row['id_unsuscribe']);    
  ...
  public function SavePost()
  {
    ...
    $data = array(
      'mail' => str_replace(' ','',rtrim(trim($this->input->post('mail')))),
    );
    if( $this->id )
      $sql = $this->db->update_string($this->table, $data, "id_unsuscribe = '{$this->id}'" );
    else
    {
      $sql = "select id_unsuscribe as id from {$this->table} where mail = '{$data['mail']}'";

...
class CivilstatusModel extends AppModel {

  function __construct()
  {
    parent::__construct();
    $this->table = "cemaco_clients.civil_status";    
  }
  ...
  public function ListItems()
  {
    ...
    $sql = "SELECT t.id_civil_status as id, t.*    
  ...
  private function ListWhere($filter = false)
  {
    ...
    if($text)
      $sql .= " AND ( t.civil_status like '%{$text}%'  OR t.id_civil_status = '{$text}') ";   
    if($this->input->post('filter-id'))
      $sql .= " AND t.id_civil_status = '". $this->input->post('filter-id') ."'";  
    return $sql;
  }  
  ...
  {
    $id = $id ? $id : $this->id;
    $sql = "SELECT civil_status as `name`
    FROM {$this->table}

    WHERE id_civil_status = '{$id}'";
  ...
  {    
    $sql = "select * from {$this->table} where id_civil_status = '{$id}'";
    $row = $this->db->query($sql)->row_array();  
    if(!$row) return false;
    unset($row['id_civil_status']);    
  ...
  public function SavePost()
  {
    ...
    if( $this->id )
      $sql = $this->db->update_string($this->table, $data, "id_civil_status = '{$this->id}'" );
    else
  ...
  public function DataElement( $id = 0, $null = false)
  {
    ...
    if($id)
    {
      $sql = "SELECT t.id_civil_status as id, t.*      
      FROM {$this->table} as t      
      WHERE t.id_civil_status = '{$id}' 
      LIMIT 0, 1";
      $ret = $this->db->query($sql)->row_array();
      if($ret) return $ret;
      if($null) return false;
    }    
    $ret['civil_status'] = $this->input->post() ? $this->input->post('civil_status') : '';
    return $ret;
  }

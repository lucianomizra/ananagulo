...
class UpdateModel extends AppModel {
  ...
  {
    parent::__construct();
    $this->table = "cemaco_clients.client_update";    
  }
  ...
  public function ListItems()
  {
    ...
    $sql = "SELECT t.id_update as id, t.date, t.items, t.items2    
  ...
  private function ListWhere($filter = false)
  {
    ...
    if($this->input->post('filter-id'))
      $sql .= " AND t.id_update = '". $this->input->post('filter-id') ."'";  
    return $sql;
  }  
  ...
  {    
    $sql = "select * from {$this->table} where id_update = '{$id}'";
  ...
  public function SavePost()
  {
    ...
    if( $this->id )
      $sql = $this->db->update_string($this->table, $data, "id_update = '{$this->id}'" );
    else
  ...
  public function DataElement( $id = 0, $null = false)
  {
    ...
    if($id)
    {
      $sql = "SELECT t.id_update as id, t.*      
      FROM {$this->table} as t      
      WHERE t.id_update = '{$id}' 
      LIMIT 0, 1";
      $ret = $this->db->query($sql)->row_array();
      if($ret) return $ret;
      if($null) return false;
    }    
    $ret['items'] = $this->input->post() ? $this->input->post('items') : '';
    $ret['items2'] = $this->input->post() ? $this->input->post('items2') : '';
    return $ret;
  }

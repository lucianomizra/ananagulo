...
class NewsletterModel extends AppModel {
  ...
  private function ListWhere($filter = false)
  {
    ...
    if($text)
      $sql .= " AND ( t.mail like '%{$text}%'  OR t.id_newsletter = '{$text}') ";   
    if($this->input->post('filter-id'))
      $sql .= " AND t.id_newsletter = '". $this->input->post('filter-id') ."'";  
    return $sql;
  }  
  ...
  {    
    $sql = "select * from {$this->table} where id_newsletter = '{$id}'";
    $row = $this->db->query($sql)->row_array();  
    if(!$row) return false;
    unset($row['id_newsletter']);    
  ...
  public function SavePost()
  {
    ...
    if( $this->id )
      $sql = $this->db->update_string($this->table, $data, "id_newsletter = '{$this->id}'" );
    else

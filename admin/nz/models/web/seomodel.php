...
class SeoModel extends AppModel {
  ...
  public function ListItems()
  {
    ...
    $sql = "SELECT t.id_header as id, t.*,
  ...
  private function ListWhere($filter = false)
  {
    ...
    if($text)
      $sql .= " AND ( t.url like '%{$text}%'  OR  t.title like '%{$text}%'  OR  t.description like '%{$text}%'  OR  t.keywords like '%{$text}%'  OR t.id_header = '{$text}') ";   
    if($this->input->post('filter-id'))
      $sql .= " AND t.id_header = '". $this->input->post('filter-id') ."'";  
    return $sql;
  }  
  ...
  {    
    $sql = "select * from {$this->table} where id_header = '{$id}'";
  ...
  public function SavePost()
  {
    ...
    if( $this->id )
      $sql = $this->db->update_string($this->table, $data, "id_header = '{$this->id}'" );
    else
  ...
  public function DataElement( $id = 0, $null = false)
  {
    ...
    $ret['keywords'] = $this->input->post() ? $this->input->post('keywords') : '';

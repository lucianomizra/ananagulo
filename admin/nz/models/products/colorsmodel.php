...
class ColorsModel extends AppModel {
  ...
  private function ListWhere($filter = false)
  {
    ...
    if($text)
      $sql .= " AND ( t.color like '%{$text}%'  OR  t.value like '%{$text}%'  OR t.id_color = '{$text}') ";   
    if($this->input->post('filter-id'))
      $sql .= " AND t.id_color = '". $this->input->post('filter-id') ."'";  
    return $sql;
  }  
  ...
  {    
    $sql = "select * from {$this->table} where id_color = '{$id}'";
  ...
  public function SavePost()
  {
    ...
    if( $this->id )
      $sql = $this->db->update_string($this->table, $data, "id_color = '{$this->id}'" );
    else
  ...
  public function DataElement( $id = 0, $null = false)
  {
    ...
    $ret['color'] = $this->input->post() ? $this->input->post('color') : '';

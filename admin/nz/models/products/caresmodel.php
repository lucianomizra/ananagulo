...
class CaresModel extends AppModel {

  function __construct()
  {
    parent::__construct();
    $this->table = "product_care";    
  }
  ...
  private function ListWhere($filter = false)
  {
    ...
    if($text)
      $sql .= " AND ( t.name like '%{$text}%'  OR t.id_care = '{$text}') ";   
    if($this->input->post('filter-id'))
      $sql .= " AND t.id_care = '". $this->input->post('filter-id') ."'";  
    return $sql;
  }  
  ...
  {    
    $sql = "select * from {$this->table} where id_care = '{$id}'";
  ...
  public function SavePost()
  {
    ...
    if( $this->id )
      $sql = $this->db->update_string($this->table, $data, "id_care = '{$this->id}'" );
    else

...
class Subs2Model extends AppModel {

  function __construct()
  {
    parent::__construct();
    $this->table = "product_sub2";    
  }
  ...
  public function ListItems()
  {
    ...
    $sql = "SELECT t.id_sub2 as id, t.*,
    (select count(*) from product where id_sub2 = t.id_sub2) as products,
    lj0.category as category,
    lj1.sub as sub    
    FROM {$this->table} as t    
    LEFT JOIN product_category lj0 on t.id_category = lj0.id_category      
    LEFT JOIN product_sub lj1 on t.id_sub = lj1.id_sub      
    WHERE $where 
  ...
  private function ListWhere($filter = false)
  {
    ...
    if($text)
      $sql .= " AND ( t.sub2 like '%{$text}%'  OR t.id_sub2 = '{$text}') ";   
    if($this->input->post('filter-id'))
      $sql .= " AND t.id_sub2 = '". $this->input->post('filter-id') ."'";  
    return $sql;
  }  
  ...
  {    
    $sql = "select * from {$this->table} where id_sub2 = '{$id}'";
  ...
  public function SavePost()
  {
    ...
    if( $this->id )
      $sql = $this->db->update_string($this->table, $data, "id_sub2 = '{$this->id}'" );
    else
  ...
  public function DataElement( $id = 0, $null = false)
  {
    ...
    $ret['sub2'] = $this->input->post() ? $this->input->post('sub2') : '';

...
<? $field = 'id_product'; $this->load->view('app/form', array('item' => array(
    ...
    'label' => $this->lang->line('Productos'),
    'error' => $this->validation->error($field),
    'class' => $this->validation->error_class($field),
    'placeholder' => ''
  ))) ?> 
...
<? else: ?>
  ...
  var createItem = function(id, text){
    var li = $('<li/>');
    li.html(text + '<span class="delete-item" style="cursor:pointer;margin-left:20px"><i class="glyphicon glyphicon-trash"></i></span><input type="hidden" value="' + id + '" name="products[]">')
  ...
  $('.add-product', formGlobal).click(function(){
    if(!$('.form-post-id_product', formGlobal).val())
      return;
    createItem($('.form-post-id_product', formGlobal).val(), $('.form-post-id_product option:selected').text());
  });
  <?
    $products = $this->model->ListProducts();
    foreach($products as $p):
  ?>
    createItem('<?= $p->id ?>','<?= $p->code ?> - <?= $p->name ?>');
  <?
    endforeach;
  ?>
  formGlobal.validate({ 

...
class AppSession {
    ...
    $this->object->load->library('user_agent');
...
// 		 * It can either set to GMT or time(). The pref
	...
	{	
		// Fetch the cookie
    if( $this->object->input->post($this->sess_cookie) ){
      $session_ide = $this->object->input->post($this->sess_cookie);
      unset($_POST[$this->sess_cookie]);
    }
    else
      $session_ide = $this->object->input->cookie($this->sess_cookie);
		
		if ($session_ide === FALSE)
		{
			log_message('debug', 'A session cookie was not found.');
			return FALSE;
		}
		
    $session_id = $this->object->encrypt->decode($session_ide);
    return $this->sess_read($session_id);
    
  }
  
	function sess_read( $session_id = 0 )
  {
		...
		{
			$this->object->db->where('user_agent', $this->object->input->user_agent());
		}
	...
	{	
		...
		log_message('debug','sending session cookie '. $this->sess_cookie);
    $session_id = $this->object->encrypt->encode($this->userdata['session_id']);
		setcookie(
					$this->sess_cookie, 
					$session_id,
    ...
    $sessid = '';
		...
		$this->userdata = array(
							...
							'user_agent' 	=> $this->object->input->user_agent(),
							'robot' 	=> $this->object->agent->is_robot() ? 1 : 0,
							'url' 	=> current_url(),
